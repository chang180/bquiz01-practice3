...
<div style="width:99%; height:87%; margin:auto; overflow:auto; border:#666 1px solid;">
    ...
    foreach($rows as $row){
        ...
        ?>
                <tr class="cent">
                    <td><img src="img/<?=$row['name'];?>" style="width:150px;height:100px"></td>
                    <td><input type="checkbox" name="sh[]" value="<?=$row['id'];?>" <?=($row['sh'])?'checked':'';?>></td>
                    <td><input type="checkbox" name="del[]" value="<?=$row['id'];?>"></td>
                    <td><input type="button" value="更新動畫" onclick="op('#cover','#cvr','./modal/upload_<?=$table;?>.php?table=<?=$table;?>&id=<?=$row['id'];?>')"></td>
